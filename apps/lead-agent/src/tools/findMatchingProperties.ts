...
export const findMatchingProperties: ToolDefinition<z.infer<typeof schema>> = {
  ...
  execute: async (db, input) => {
    ...
    if (!lead.budget_max && !lead.location_pref && !lead.property_interest) {
      return { status: "insufficient_profile" as const };
    }

    const all = await listProperties(db);
    const preferredTier = lead.segment === "client" ? "upgrade" : "standard";

    const matches = all.filter((p) => {
      if (lead.budget_max && p.price > lead.budget_max * 1.1) return false;
