...
import { getLead, getEngagementEvent, updateLead } from "../db/queries.js";
...
const schema = z.object({
  ...
  evidence_interaction_id: z.string().uuid(),
});
...
export const reactivateLead: ToolDefinition<z.infer<typeof schema>> = {
  ...
  execute: async (db, input) => {
    ...
    const evidence = await getEngagementEvent(db, input.evidence_interaction_id);
    if (!evidence) {
    ...
    if (ageMs > maxAgeMs) {
      throw new ToolError(
        "EVIDENCE_INVALID",
        `Interaction ${input.evidence_interaction_id} is from ${evidence.created_at}, older than the ${REACTIVATION_EVIDENCE_MAX_AGE_DAYS}-day freshness window required for reactivation.`
      );
    }

    await updateLead(db, lead.id, { stage: "contacted" });
